 ...
 */
        ...
        Map<Integer,Map<String,Object>> myCar = new HashMap<>();
        cars.put(1, car1);
        cars.put(2, car2);
        cars.put(3, car3);
        ...
        for(Map<String, Object> each : cars.values()){
            System.out.println(each.get("brand") + "-" + each.get("model") + "-" + each.get("price"));
        }

        //4.3 Display all the electric cars
        System.out.println();
        for(Map<String, Object> each : cars.values()){
            if((boolean) each.get("electric")){  // each.get("electric").equals(true)
